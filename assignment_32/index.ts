...
let current_users: string[] = ['aliraza', 'muneeb', 'zohaid', 'atif', 'noman'];
let new_users: string[] = ['aliraza', 'khalil', 'ukasha', 'noman', 'bilal'];

let current_users_lower: string[] = current_users.map((user) => user.toLowerCase());

for (const user of new_users) {
    if (current_users_lower.indexOf(user.toLowerCase()) !== -1) {
        console.log(`${user}, You have to enter a new username.`);
    } else {
